...
# Dir-path
logs_train_dir = "F://Githubtest//"
data_dir = "E:/PycharmProjects/PHI Challenge 2018/Task8/Edataset/"
model_save_dir = "F://Githubtest//"

# Model-parameters
num_classes = 4
seg_ratio = 0.8
opt = 'sgd'  #['sgd', 'rmsprop', 'adagrad', 'adadelta', 'adam']
# Hyper-parameters
MAX_STEP = 400000
BATCH = 32
...
def create_optimizer(opt, learning_rate,global_step ,decayrate=0.96):
    """
    ...
    """
    learning_rate = tf.train.exponential_decay(learning_rate, global_step, decay_steps=Sample_Size / BATCH,
                                               decay_rate=decayrate,
                                               staircase=True)
    assert opt in ['sgd', 'rmsprop', 'adagrad', 'adadelta', 'adam', 'adamax', 'nadam']
    if opt == 'sgd':
        optimizer = tf.train.GradientDescentOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    elif opt == 'rmsprop':
        optimizer = tf.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    elif opt == 'adagrad':
        optimizer = tf.train.AdagradOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    elif opt == 'adadelta':
        optimizer = tf.train.AdadeltaOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    elif opt == 'adam':
        optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    else:
        optimizer = None
    return optimizer
...
if __name__ == '__main__':
    ...
    with tf.name_scope('loss'):
        trian_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=problabel, labels=y_imgs))
        tf.summary.scalar('loss', trian_loss)

    with tf.name_scope('valid_loss'):
        valid_loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(logits=problabel, labels=y_imgs))
    ...
    #                                            staircase=True)
    # optimizer = tf.train.AdadeltaOptimizer(learning_rate=learning_rate).minimize(trian_loss, global_step)
    optimizer = create_optimizer(opt,Learning_Rate,global_step)
    ...
    vgg.load_weights('E:\PycharmProjects\DeepLearing-Classification\data\\vgg16_weights.npz', sess)
    saver = vgg.saver()
    ...
    for step in np.arange(MAX_STEP):
        ...
        summary, _, trianacc, trainloss = sess.run([merged, optimizer, trian_acc, trian_loss],
                                                   feed_dict={x_imgs: image, y_imgs: labels, keep_prob: keep_prob_val})
